...
bio = "poa_obitos_2023.csv"
meteoro = "dados_A801_H_2023-01-01_2024-12-31.csv"
...
bio["data"] = bio["data"].astype(str).str.zfill(8)
bio["data"] = pd.to_datetime(bio["data"], format = "%d%m%Y", errors = "coerce")
bio["obito"] = np.ones(len(bio))
bio = bio.groupby(["data"], dropna = True).sum()
bio.drop(columns = "causa_base", inplace = True)
#
meteoro.drop(columns = ["Unnamed: 8"], inplace = True)
meteoro = meteoro.rename(columns = {'Data Medicao' : "data", 'Hora Medicao' : "hora",
									...
									'VENTO, VELOCIDADE HORARIA(m/s)' : "ventovel"})
...
meteoro["tmin"] = pd.to_numeric(meteoro["tmin"], errors = "coerce")
meteoro["tmed"] = pd.to_numeric(meteoro["tmed"], errors = "coerce")
meteoro["tmax"] = pd.to_numeric(meteoro["tmax"], errors = "coerce")
meteoro["urmax"] = pd.to_numeric(meteoro["urmax"], errors = "coerce")
meteoro["urmin"] = pd.to_numeric(meteoro["urmin"], errors = "coerce")
meteoro["ventovel"] = pd.to_numeric(meteoro["ventovel"], errors = "coerce")
#prec = meteoro.groupby(by = ["data"])["prec"].sum()
maxi = meteoro.groupby(by = ["data"]).max()
maxi.drop(columns = ["tmed", "tmin", "urmin", "ventovel", "hora"], inplace = True)
maxi.reset_index(inplace = True)
mini = meteoro.groupby(by = ["data"]).min()
mini.drop(columns = ["tmed", "tmax", "urmax", "ventovel", "hora"], inplace = True)
mini.reset_index(inplace = True)
medi = meteoro.groupby(by = ["data"]).mean().round(2)
medi.drop(columns = ["tmax", "tmin", "urmin", "urmax", "hora"], inplace = True)
medi.reset_index(inplace = True)
meteoro = meteoro.groupby(by = ["data"]).mean()
meteoro.reset_index(inplace = True)
meteoro.drop(columns = ["tmax", "tmin", "tmed", "urmin", "urmax", "hora", "ventovel"], inplace = True)
meteoro = meteoro.merge(mini, on = "data", how = "inner")
meteoro = meteoro.merge(medi, on = "data", how = "inner")
meteoro = meteoro.merge(maxi, on = "data", how = "inner")
posicao_colunas = ["data", "tmin", "tmed", "tmax", "urmin", "urmax", "ventovel"]
meteoro = meteoro[posicao_colunas]
meteoro["ventovel"] = meteoro["ventovel"] * 3.6 # m/s >> km/h
Tc = meteoro["tmed"] # T (C) < 10
Tf = ((meteoro["tmed"] * 9) / 5) + 32  # C >> F
Tf_WC = ((meteoro["tmin"] * 9) / 5) + 32  # C >> F 
Tf_HI = ((meteoro["tmax"] * 9) / 5) + 32  # C >> F 
Vkmh = meteoro["ventovel"] * 3.6 # ms >> km/h # V (km/h) > 4.82
Vmph = meteoro["ventovel"] * 2.237 # ms >> mph 
RH = meteoro["urmin"] # % Umidade Média ??
meteoro["wind_chill"] = 35.74 + 0.6215 * Tf_WC - 35.75 * Vmph**0.16 + 0.4275 * Tf_WC * Vmph**0.16 # F mph # 
meteoro["heat_index"] =  -42.379 + 2.04901523*Tf_HI + 10.14333127*RH - .22475541*Tf_HI*RH - .00683783*Tf_HI*Tf_HI - .05481717*RH*RH + .00122874*Tf_HI*Tf_HI*RH + .00085282*Tf_HI*RH*RH - .00000199*Tf_HI*Tf_HI*RH*RH
meteoro[["heat_index", "wind_chill"]] = (meteoro[["heat_index", "wind_chill"]] - 32) * 5 / 9 # ºF >> ºC
meteoro[["heat_index", "wind_chill"]] = meteoro[["heat_index", "wind_chill"]].round(2)
print(f"\n{green}MÍNIMOS:\n{reset}{mini}\n")
print(f"\n{green}MÉDIOS:\n{reset}{medi}\n")
print(f"\n{green}MÁXIMOS:\n{reset}{maxi}\n")
print(f"\n{green}DADOS METEOROLÓGICOS:\n{reset}{meteoro}\n")
print(f"\n{green}COLUNAS DE DADOS METEOROLÓGICOS:\n{reset}{meteoro.columns}\n")
print(f"\n{green}DADOS BIOLÓGICOS:\n{reset}{bio}\n")
print(f"\n{green}COLUNAS DE DADOS BIOLÓGICOS:\n{reset}{bio.columns}\n")
sys.exit()
